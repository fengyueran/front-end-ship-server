...
配置哪些文件不需要编译，使用 include 引入的文件可以使用 exclude 属性过滤。 而，通过 files 属性明确指定的文件却总是会被包含在内(忽略 exclude)。 如果不指定， exclude 默认情况下会排除 node_modules，bower_components，jspm_packages 和`outDir`配置属性指定的目录。
