...
- ArrayBuffer 的长度是固定的，而数组大小可以自由增减
- ArrayBuffer 数据存放在栈中(取数据时更快)，而数组放在堆中
- ArrayBuffer 没有 push/pop 等方法
- ArrayBuffe 只能读不能写，写需要借助 TypeArray/DataView

#### [TypeArray](https://www.apiref.com/javascript-zh/Reference/Global_Objects/TypedArray.htm)

ArrayBuffer 存储了原始的字节序列，这些字节序列表示什么可以通过类似数组的视图 TypeArray 来解释
诸如 Uint8Array、Uint16Array 都属于 TypeArray，但并没有一个叫 TypeArray 的构造器。
...
- new TypedArray(object)
  ...
  当传入一个 object 作为参数时，如同通过 [TypedArray.from()](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/TypedArray/from) 方法一样创建一个新的类型数组。如果给定的是 Array，或任何类数组对象则会创建一个相同长度的类型化数组，并复制其内容。
...
// 现在在偏移量为 0 处获取 16 位数字，它由 2 个字节组成，一起解析为 65535(1的二进制为00000001，2的二进制00000010，16位就是0000000100000010，转换为十进制就是258)
